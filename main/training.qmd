...
def get_primary_labels(labels: dict, id2label: dict) -> list:
    """
    Multi-label derivative classification logic.
    ...
    # --- Special types ---
    if labels.get("warr"):
        results.append(20 if labels.get("hist") else 19)  # Current=19, Historic=20
    if labels.get("emb"):
        results.append(22 if labels.get("hist") else 21)  # Current=21, Historic=22

    # --- Use / Speculative (need gen) ---
    if labels.get("gen"):
        ...
        if not results and labels.get("spec"):
            for hedge_type in ["ir", "fx", "cp", "eq", "gen"]:
                if labels.get(hedge_type):
                    results.append(hedge_map[hedge_type][2])

    # --- Context-only (no _use/spec, or gen=0) ---
    if not results:
        for hedge_type in ["ir", "fx", "cp", "eq", "gen"]:
            if labels.get(hedge_type):
    ...
    # --- Irrelevant ---
    if labels.get("irr"):
        results.append(24)  # corrected to match your label map

    # --- Deduplicate + sort ---
    results = sorted(set(results))
    return [id2label[r] for r in results]
