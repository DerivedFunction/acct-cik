...
wp = execute_sql("SELECT * FROM post_results", fetch=True)
wp.head()
```
...
dd = execute_sql("DELETE FROM post_results", fetch=False)
print(dd)
```

## Save a dataframe as an Excel file
```{python}
wp.to_excel("post_results.xlsx", index=False)
```
```{python}
wf.to_excel("webpage_results.xlsx", index=False)
```