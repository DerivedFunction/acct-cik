...
REPORT_RESULTS_EXCEL_PATH = "../excel/report_urls.xlsx"
...
def rate_limit_sleep():
    ...
    time.sleep(NUM_THREADS / SEC_RATE_LIMIT)
...
def extract_content(data: str, asHTML=True, max_len=2000) -> str:
    ...
    if asHTML:
        ...
        text = keep_allowed_chars(data, False)
...
def fetch_all_grouped(
    df: pd.DataFrame, saveIteration: int = 10, max_workers: int = 5
) -> pd.DataFrame:
    ...
    with ThreadPoolExecutor(max_workers=max(2, NUM_THREADS)) as executor:
        # Submit all tasks
        future_to_cik = {
