...
NUM_THREADS = 4
SEC_RATE_LIMIT = 1/10  # requests per second
```
...
def save_batch_report_urls(df):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        try:
            df = df[["cik", "year", "url"]]
            df["status"] = (
                1 if "status" not in df.columns else df["status"]
...
def fetch_all_grouped(saveIteration: int = 10, max_workers: int = 5
) -> pd.DataFrame:
    ...
    if existing_report_df is None or existing_report_df.empty:
        existing_report_df = pd.DataFrame(columns=["cik", "fyear"])

    already_done = set(zip(existing_report_df["cik"], existing_report_df["fyear"]))
    # group: {cik -> [fyears]}
    cik_groups = existing_report_df.groupby("cik")["fyear"].apply(list).reset_index()
    ...
    def process_cik(row):
        ...
        for fyear in years_to_fetch:
            ...
            for filing in year_filings:
                cik_records.append({"cik": cik, "year": fyear, **filing, "status": 1})

        # If we don't have the fyears, we use dummy urls and set status to -1
        for year in years:
            if (cik, year) not in already_done:
                cik_records.append({"cik": cik, "year": year, "url": "", "status": -1})

        return cik_records

    # Process CIKs concurrently
    total_ciks = len(cik_groups)
    SEC_RATE_LIMIT = NUM_THREADS / 10  # requests per second
