...
REPORT_RESULTS_EXCEL_PATH="../excel/final_report_results.xlsx"
...
def filter_by_keywords(content: str, year: int) -> dict:
    """
    Finds paragraphs matching keywords, chunks them into overlapping
    sentence groups, and returns them.
    """
    keywords_lower = [kw.lower() for kw in ADDITIONAL_KEYWORDS]
    ...
    # Use the year to ensure chunking is consistent for the same report
    random.seed(year)

    # Correctly split content into paragraphs using double newlines
    paragraphs = content.split("\n\n")
    match_chunks = []
    seen_matches = set()

    for paragraph in paragraphs:
        # Normalize paragraph by removing excess whitespace
        ...
        # Skip paragraph if it contains ignore words or no keywords
        if any(kw in normalized_paragraph for kw in ignore_keywords):
            continue
        if not matches_text(normalized_paragraph):
            continue

        # Split the relevant paragraph into sentences
        sentences = [s.strip() for s in re.split(r'(?<=[.?!])\s+', paragraph) if s.strip()]

        # **New Chunking Logic**
        # Group sentences into random-sized chunks (4 to 6) with overlap
        i = 0
        while i < len(sentences):
            chunk_size = random.randint(4, 6)
            # Create a chunk from the current sentence to the end of the chunk size
            chunk = ". ".join(sentences[i : i + chunk_size])
            
            if chunk:
                normalized_chunk = chunk.lower()
                if normalized_chunk not in seen_matches:
                    seen_matches.add(normalized_chunk)
                    match_chunks.append(chunk)
            
            # Move the starting point forward, creating a 1-sentence overlap
            i += chunk_size - 1

    return {"matches": match_chunks}
```
