...
`webpage_data` stores the cache of the 10-K report. `webpage_result` stores the processed data from server and keyword search.
...
def extract_content(data: str, asHTML=True) -> str:
    ...
    for para in paragraphs:
        ...
        # Strip multiple symbols but keep single ones such as '-----' but not '-'
        para = re.sub(r"(-{3,}|={3,}|\.{3,})", "", para)

        # Remove table of contents at the bottom of page
        para = re.sub(r"\ntable of contents", "", para, flags=re.IGNORECASE)
...
def filter_by_keywords(content: str, year: int) -> dict:
    ...
    for paragraph in paragraphs:
        ...
        sentences = [s.strip() for s in re.split(r'(?<![A-Z0-9])\s*\.\s*(?![a-zA-Z0-9])', paragraph) if s.strip()]
...
def store_webpage_data():
    with ThreadPoolExecutor(max_workers=NUM_THREADS) as executor:
        futures = [
            executor.submit(process_url, report.link, report.cik, report.year, True)
...
merged = kw_summary.merge(
    sa.drop(columns=["total_sentences", "cik", "year", "Irrelevant / unrelated context"]), 
    on=["url"], how="left") 
