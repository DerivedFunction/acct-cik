...
from concurrent.futures import ThreadPoolExecutor, ProcessPoolExecutor, as_completed # Concurrency
...
NUM_THREADS = 1
NUM_PROCESSES = (mp.cpu_count() - 2)/NUM_THREADS
...
# Pre-compiled regex patterns for each category (mega-patterns)
IR_REGEX = re.compile(
...
def extract_filings(data: dict, cik: str, name: str, ticker: str) -> List[dict]:
    ...
    for i, f_type in enumerate(forms):
        if f_type in FILING_TYPES:
            ...
            doc = primary_docs[i]
            if not doc or doc.endswith("txt") or doc.endswith("0001.htm"):  # For old files
...
def clean_paragraph(para: str) -> str:
    """Clean a single paragraph with all regex and formatting rules"""
    para = para.strip()
    if not para:
        return ""
...
def merge_paragraphs_chunk(chunk: list) -> list:
    """Merge paragraphs within a chunk using bullet/numbered/punctuation rules"""
    merged = []
    i = 0
    while i < len(chunk):
        line = chunk[i]
        if BULLET_PATTERN.match(line):
            if len(line.strip()) == 1 and i + 1 < len(chunk):
                line = f"{line} {chunk[i + 1]}"
                i += 1
            if merged and BULLET_PATTERN.match(merged[-1]):
                merged[-1] += f"\n{line}"
            else:
                merged.append(line)
        elif NUMBERED_PATTERN.match(line):
            if merged and NUMBERED_PATTERN.match(merged[-1]):
                merged[-1] += f"\n{line}"
            else:
                merged.append(line)
        elif merged and not PUNCTUATION_END_PATTERN.search(merged[-1]):
            merged[-1] += f" {line}"
        else:
            merged.append(line)
        i += 1
...
def extract_content(data: str, asHTML=True, max_len=2000) -> str:
    ...
    if asHTML:
        ...
        # Parallel merge
        chunk_size = max(50, len(paragraphs) // 12)
        chunks = [paragraphs[i:i + chunk_size] for i in range(0, len(paragraphs), chunk_size)]

        with ProcessPoolExecutor(max_workers=NUM_PROCESSES) as executor:
            merged_chunks = list(tqdm(executor.map(merge_paragraphs_chunk, chunks), total=len(chunks), desc="Merging paragraphs"))

        merged_paragraphs = [para for chunk in merged_chunks for para in chunk]
    ...
    # Parallel cleaning
    with ProcessPoolExecutor(max_workers=NUM_PROCESSES) as executor:
        cleaned = list(tqdm(executor.map(clean_paragraph, paragraphs), total=len(paragraphs), desc="Cleaning paragraphs"))
    ...
    for para in cleaned:
        if not para:
            continue
        if len(para) < 15 and final_output:
            final_output[-1] = f"{final_output[-1]} {para}"
        else:
...
def process_sentence(args):
    """Worker function for parallel sentence processing"""
    (
        i,
        sentence,
        all_sentences,
    ...
    def expand_context(all_sentences, target_idx, target_category):
    ...
    if category and category != "gen":
        ...
        if should_ignore(normalized):
            return None
        return (category, normalized, final_sentence, i)
    return None
...
def filter_by_keywords(
    ...
    all_sentences = [re.sub(r"\s+", " ", s.strip()) for s in raw_sentences]

    categorized_matches = {k: [] for k in ["ir", "fx", "cp", "spec", "gen"]}
    seen_matches = {k: set() for k in categorized_matches}
    seen_sentences_global = set()

    # First pass: IR, FX, CP, SPEC
    args_list = [
        (i, s, all_sentences, year, min_char_length, max_char_length, ignore_keywords)
        for i, s in enumerate(all_sentences)
    ]

    with ProcessPoolExecutor(max_workers=NUM_PROCESSES) as executor:
        for result in executor.map(process_sentence, args_list):
            if result:
                category, normalized, final_sentence, i = result
    ...
    for i, sentence in enumerate(all_sentences):
        if len(sentence.split()) < 6 or i in seen_sentences_global:
            continue
        category = None
        for cat, regex in CATEGORY_REGEX_ORDER:
            if regex.search(sentence):
                category = cat
                break
        if category == "gen":
            final_sentence = process_sentence(
                (
                    i,
                    sentence,
                    all_sentences,
            ...
            if final_sentence:
                _, normalized, final_text, _ = final_sentence
                if normalized not in seen_matches["gen"]:
                    seen_matches["gen"].add(normalized)
                    categorized_matches["gen"].append(final_text)
...
def process_all_reports_fully():
    ...
    def process_keywords_for_report(report):
        ...
        try:
            ...
            sentences = filter_by_keywords(content, int(report.year))
            matches = []
