...
NUM_THREADS = 4
NUM_PROCESSES = max(1, mp.cpu_count() // NUM_THREADS)
...
def extract_content(data: str, asHTML=True, max_len=2000) -> str:
    ...
    if asHTML:
        ...
        with ProcessPoolExecutor(max_workers=NUM_PROCESSES) as executor:
            merged_chunks = executor.map(merge_paragraphs_chunk, chunks)
    ...
    # Parallel cleaning
    with ProcessPoolExecutor(max_workers=NUM_PROCESSES) as executor:
        cleaned = executor.map(clean_paragraph, paragraphs)
...
def filter_by_keywords(
    ...
    with ProcessPoolExecutor(max_workers=NUM_PROCESSES) as executor:
        for result in executor.map(process_sentence, gen_args_list):
            if result:
                _, normalized, final_text, _ = result
...
def process_all_reports_fully():
    ...
    results = 0
    empty_results = 0
    SEC_RATE_LIMIT = NUM_THREADS / 8  # requests per second
    with ThreadPoolExecutor(max_workers=NUM_THREADS) as executor:
        future_to_report = {
