...
# OPTIMIZED FOR N CORES
NUM_PROCESSES = mp.cpu_count() - 1  # Leave 1 core for system
...
def process_all_reports_fully():
    """
    OPTIMIZED VERSION: Uses ProcessPoolExecutor for true parallelism.
    Takes advantage of all 44 CPU cores.
    """
    ...
    print(f"Processing {len(reports_to_process)} new reports")
    print(f"Already processed: {len(processed_set)} reports")

    results = 0
    empty_results = 0
    
    # Use ProcessPoolExecutor with all 40 workers
    with ProcessPoolExecutor(max_workers=5) as executor:
        future_to_report = {
            executor.submit(process_keywords_for_report_standalone, r.url, r.cik, r.year): r
    ...
    print(f"✓ Processed {results} reports successfully")
    print(f"✗ Empty/failed: {empty_results} reports")
```
...
fetch_all_grouped() # Fetches all the 10-K Links
```

```{python}
process_all_reports_fully() # Extracts the content
```