...
IR_KEYWORDS = MATCHING_KEYWORDS[8]
FX_KEYWORDS = MATCHING_KEYWORDS[10]
CP_KEYWORDS = MATCHING_KEYWORDS[12]
GEN_KEYWORDS = MATCHING_KEYWORDS[0]  # Last resort matching (gerneric usage)
SPEC_KEYWORDS = MATCHING_KEYWORDS[2] # Last resort (speculative/policy)
DL_KEYWORDS = MATCHING_KEYWORDS[4]
ED_KEYWORDS = MATCHING_KEYWORDS[6]
KEYWORDS = []
KEYWORDS.extend(IR_KEYWORDS)
KEYWORDS.extend(FX_KEYWORDS)
KEYWORDS.extend(CP_KEYWORDS)
KEYWORDS.extend(GEN_KEYWORDS)
KEYWORDS.extend(DL_KEYWORDS)
KEYWORDS.extend(ED_KEYWORDS)
```
...
def save_process_result(df):
    ...
    c = conn.cursor()
    c.execute(
        "INSERT OR REPLACE INTO webpage_result (url, ir, fx, cp, gen, spec) VALUES (?, ?, ?, ?, ?, ?)",
        (
            df.url,
            json.dumps(df.ir),
            json.dumps(df.fx),
...
def filter_by_keywords(
    ...
    FX_KEYWORDS = MATCHING_KEYWORDS[10]
    CP_KEYWORDS = MATCHING_KEYWORDS[12]
    GEN_KEYWORDS = MATCHING_KEYWORDS[0]  # Last resort matching (generic usage)
    SPEC_KEYWORDS = MATCHING_KEYWORDS[2] # Last resort (speculative/policy)
    IGNORE_KEYWORDS # Do not match or add to context
    """
    ignore_keywords = [kw.lower() for kw in IGNORE_KEYWORDS]

    # Prepare keyword sets for each category
    ir_keywords = [kw.lower() for kw in MATCHING_KEYWORDS[8]]
    fx_keywords = [kw.lower() for kw in MATCHING_KEYWORDS[10]]
    cp_keywords = [kw.lower() for kw in MATCHING_KEYWORDS[12]]
    gen_keywords = [kw.lower() for kw in MATCHING_KEYWORDS[0]]
    spec_keywords = [kw.lower() for kw in MATCHING_KEYWORDS[2]]

    debug_print("Generating sentences")

    def get_keyword_category(text: str) -> str:
        """Determine which keyword category the text matches (priority order: IR > FX > CP > SPEC > GEN)"""
        normalized = text.lower()

        # Check in priority order: IR > FX > CP > SPEC > GEN (generic last)
        if any(kw in normalized for kw in ir_keywords):
            return "ir"
        elif any(kw in normalized for kw in fx_keywords):
            return "fx"
        elif any(kw in normalized for kw in cp_keywords):
            return "cp"
        elif any(kw in normalized for kw in spec_keywords):
            return "spec"
        elif any(kw in normalized for kw in gen_keywords):
            return "gen"
        return None
    ...
    def expand_context(
        ...
        """Expand context around target sentence, only including sentences from same category or no category"""
        ...
        while True:
            ...
            if left_idx >= 0:
                ...
                # Only add if same category or no category (None)
                if left_category is None or left_category == target_category:
                    ...
                    if measure_merged_length(candidate) <= max_char_length:
                        ...
                        left_idx = -1  # Stop trying left
                else:
                    left_idx = -1  # Stop trying left if different category
            ...
            if right_idx < len(all_sentences):
                ...
                if right_category is None or right_category == target_category:
                    ...
                    if measure_merged_length(candidate) <= max_char_length:
                        ...
                        right_idx = len(all_sentences)  # Stop trying right
                else:
                    right_idx = len(
                        all_sentences
                    )  # Stop trying right if different category
    ...
    # Process sentences in two passes: specific keywords first, then generic
    categorized_matches = {"ir": [], "fx": [], "cp": [], "spec": [], "gen": []}
    seen_matches = {"ir": set(), "fx": set(), "cp": set(), "spec": set(), "gen": set()}
    seen_sentences_global = set()  # Track all matched sentences globally
    random.seed(year)

    # First pass: Process IR, FX, CP, SPEC (specific keywords)
    for i, sentence in enumerate(all_sentences):
        if len(sentence.split()) < 4:
            continue
        # Check which category this sentence matches
        category = get_keyword_category(sentence)

        # Skip generic in first pass
        if category and category != "gen":
            ...
            # Add to appropriate category if not seen
            if normalized not in seen_matches[category]:
                ...
                seen_sentences_global.add(i)  # Mark this sentence index as used

    # Second pass: Process GEN (generic keywords) - only if not already matched
    for i, sentence in enumerate(all_sentences):
        ...
        # Only process generic in second pass
        if category == "gen":
            # Expand context around sentence
            final_sentence = expand_context(all_sentences, i, category)
            normalized = final_sentence.lower().strip()
            ...
            # Add to gen category if not seen
