...
DB_PATH = "./webpage.db"
REPORT_URLS_PATH = "../excel/report_data.csv"
...
def get_sentence_analysis():
    ...
    with pd.ExcelWriter(SERVER_EXCEL_PATH, engine="openpyxl") as writer:
        ...
        firms_label2_only = sa.groupby("cik").filter(
            lambda g: g[id2label[2]].sum() > 0 and g[exclude_cols].sum().sum() < g[id2label[2]].sum()
        )
        ...
        firms_liabilities.to_excel(writer, sheet_name="Derivative Liabilties or Warrants", index=False)
...
```{python}
def build_sentence_label_excel(output_path="./sentence_labels.xlsx"):
    # Load both DB tables
    conn = sqlite3.connect(DB_PATH)
