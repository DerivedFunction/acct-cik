...
def generate_accounting_noise_paragraph(
    ...
    if random.random() < 0.15:  # Accounting standards
        all_sentences.append(generate_other_policy_update())
    if random.random() < 0.15:  # Generat of text and numbers
    ...
    if random.random() < 0.05: # Nothing
        all_sentences.append([""])

    # Shuffle and select
    random.shuffle(all_sentences)
    selected_sentences = all_sentences[0][:max_len]
    selected_sentences = [s[0].upper() + s[1:] if s else "" for s in all_sentences]
    # Create paragraph
    paragraph = (
        f"<reportingYear>{reporting_year}</reportingYear> "
