...
def deboost_keywords_parallel(batch_size=20, max_words_per_sentence=3):
    """
    ...
    for prefix, label in swap_types.items():
        for _ in range(batch_size):
            ...
            for _ in range(length):
                ...
                if prefix == "ir" and "interest rate" in swap_type and random.random() < 0.5:
                    swap_type = re.sub("interest rate", "", swap_type)
            ...
            if swaps:
                ...
                sentences.append((" ".join(swaps), label))
    ...
    for _ in range(len(derivative_specific) * 3):
        sentence = []
        length = random.randint(2, 4)
        sentence = derivative_specific[:length]
    ...
    words_to_deboost_label_3.extend([
        ...
        "receive-equity return, pay-floating equity", "equity collar", "total return"
    ])
    ...
    for _ in range(batch_size * len(words_to_deboost_label_3) // 2):
        ...
        words_with_context = list(set(words_with_context))
        random.shuffle(words_with_context)
        sentence = " ".join(words_with_context)
