    ...
    # ============ LABELS 6 & 7: EMBEDDED DERIVATIVES ============
        ...
        for _ in range(0, max_len):
            ...
            if noncash_type == "stock_debt":
                ...
                sentence = template.format(company=company_name, shares=f"{shares:,}")
...
def generate_labeled_hedge_paragraph(
    ...
    if has_active_derivative is None:
        ...
        if random.random() < 0.8:
            ...
            all_sentences.append(template.format(company=company_name, verb=verb))
        ...
        all_sentences.append(
            nt_template.format(
                company=company_name, verb=random.choice(hedge_may_use_verbs)
            )
        )
    ...
    # Labels 0 and 1: Specific positions
        ...
        if include_policy:
            ...
            if random.random() < 0.1:
                ...
                all_sentences.append(
                    template.format(
                        company=company_name, verb=random.choice(hedge_use_verbs)
                    )
                )
        # Hedge transaction
        if random.random() < 0.1:
        ...
        # Commodity Price specific logic
            ...
            if random.random() < 0.6:
                ...
                all_sentences.append(
                    template.format(
                        company=company_name, verb=verb, commodity=commodity
                    )
                )
...
def generate_accounting_noise_paragraph(
    ...
    for _ in range(num_litigation):
        if random.random() < 0.5:
            ...
            verb = random.choice(assessment_verbs)
            sentence = template.format(company=company_name, verb=verb)
        else:
    ...
    if random.random() < 0.05:  # Generat of text and numbers
        noise = generate_sec_noise(company=company_name, currency_unit=money_units, month=random.choice(months), year=current_year)
        all_sentences = noise
