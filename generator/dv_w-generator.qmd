...
foreign_currency_translation_templates = [
    ...
    "The weakening of the {major_currency} against the {major_currency_2} resulted in an unfavorable translation impact of {currency_code}{amount} {money_unit} in {year}",
...
def generate_accounting_noise_paragraph(
    ...
    for _ in range(num_bs_changes):
        ...
        random.shuffle(sentences)
        sentences = sentences[: random.random(1, len(sentences))]
        merged_text = ". ".join(sentences)
        all_sentences.append([merged_text])
    ...
    for _ in range(num_commodity):
        ...
        if random.random() < 0.5:
            ...
            months = generate_value(False, 12)
            sentence = template.format(
                company=pick_company_name(company_name),
                pct=pct,
                months=months,
