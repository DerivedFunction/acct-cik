...
def deboost_keywords_parallel(batch_size=20, max_words_per_sentence=3):
    ...
    # --- 2. Derivative-specific phrases that could be confused (label 2) ---
    derivative_specific = [
        "equity swap",
        "forward contract",
        "futures contract",
    ...
    # --- 3. Generic derivative terms that need context (label 17 - ambiguous) ---
    gen_standalone = [
        "option",
        "collar",
        "call",
    ...
    # Generate GEN_STANDALONE combinations (label 17)
    for _ in range(len(gen_standalone) * 4):
        num_words = random.randint(1, 3)
        selected_words = random.sample(gen_standalone, min(num_words, len(gen_standalone)))
        words_with_context = []
    ...
    # --- 4. Contextless / irrelevant words (label 3) - NO GEN_STANDALONE ---
    words_to_deboost_label_3 = [
        # Interest Rate context (without derivative terms)
        "interest rate",
        ...
        "EURIBOR",
        "prime rate",
        # Foreign Exchange context (without derivative terms)
        "foreign currency",
        "currency",
        "exchange rate",
        ...
        "commodity risk",
        "price volatility",
        # Financial instrument buzzwords (without specific derivative terms)
    ...
    # Add commodities as contextless phrases (just the commodity names and prices)
    for commodity in commodities:
        words_to_deboost_label_3.extend([
            f"{commodity} price",
            f"{commodity} exposure",
            f"{commodity} inventory",
            f"{commodity} cost",
            commodity,
        ])

    # Generate label 3 sentences (contextless/irrelevant)
    for _ in range(len(words_to_deboost_label_3) * 5):
        num_words = random.randint(1, max_words_per_sentence)
        selected_words = random.sample(
            words_to_deboost_label_3, 
            min(num_words, len(words_to_deboost_label_3))
        )
        words_with_context = []
    ...
    # --- 5. Type-specific derivative terms (labels 14, 15, 16) ---
    # These are actual derivative contracts by type
        ...
        for _ in range(batch_size):
            ...
            for _ in range(length):
                ...
                if prefix == "cp" and "commodity" in swap_type:
                    swap_type = re.sub(
                        "commodity", random.choice(commodities), swap_type
                    )
            ...
            if swaps:
                ...
                if random.random() < 0.5:
                    swaps.append(f"{random.randint(1990, 2025)}")
                
                # Optionally add contextless words from label 3 (NOT gen_standalone)
                if random.random() < 0.3:
                    contextless_word = random.choice(words_to_deboost_label_3)
                    swaps.append(contextless_word)
