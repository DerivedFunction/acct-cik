...
def generate_labeled_hedge_paragraph(
    ...
    if has_active_derivative is None:
        ...
        if swapType == "gen" or swapType == "mixed":
            label = 0
        elif swapType == "ir":
            label = 8
        elif swapType == "fx":
    ...
    # Labels 0 and 1: Specific positions
        ...
        if include_policy:
            ...
            if random.random() < 0.5:
                ...
                all_sentences.append(template.format(company=company_name))
            # Determine which specific logic to apply
        if swapType == "mixed":
            # For mixed types, randomly pull from each type's templates
            active_types = mixed_types

            # Add context sentences based on active types
            for active_type in active_types:
                if random.random() < 0.4:  # Lower probability per type
                    if active_type == "fx":
            ...
            # Add impact sentences based on active types
            for active_type in active_types:
                if random.random() < 0.5:  # Lower probability per type
                    if active_type == "fx":
            ...
            # Add specific position sentences for each active type
            for active_type in active_types:
                # Label 0: Active derivative positions
                if has_active_derivative:
                    if active_type == "fx":
                        # Add FX-specific sentences with lower probability
                        if random.random() < 0.6:
                            template = random.choice(hedge_active_position_template)
                            swap_type_selected = random.choice(
                                [
                                    s
                                    for s in swap_types
                                    if s in derivative_keywords["fx"]
                                ]
                            )
                            notional = random.randint(50, 1000)
                            sentence = template.format(
                                company=company_name,
                                swap_type=swap_type_selected,
                                month=random.choice(months),
                                end_day=random.randint(28, 31),
                                year=current_year,
                                currency_code=currency_code,
                        ...
                        # Add IR-specific sentences with lower probability
                        if random.random() < 0.6:
                            ...
                            swap_type_selected = random.choice(
                                [
                                    s
                                    for s in swap_types
                                    if s in derivative_keywords["ir"]
                                ]
                            )
                        ...
                        # Add CP-specific sentences with lower probability
                        if random.random() < 0.6:
                            ...
                            swap_type_selected = random.choice(
                                [
                                    s
                                    for s in swap_types
                                    if s in derivative_keywords["cp"]
                                ]
                            )
                            commodity = random.choice(commodities)
                ...
                # Label 1: Similar logic for inactive positions
                else:
                    if active_type in ["fx", "ir", "cp"]:
                        if random.random() < 0.5:
                            ...
                            swap_type_selected = random.choice(
                                [
                                    s
                                    for s in swap_types
                                    if s in derivative_keywords[active_type]
                                ]
                            )
                            term_year = (
                                random.choice(past_years)
    ...
    if len(all_sentences) < max_len:
        print("Too short, regenerating")
        return generate_labeled_hedge_paragraph(
            has_active_derivative=has_active_derivative, swapType=swapType
        )
