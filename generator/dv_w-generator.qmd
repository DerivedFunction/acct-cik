...
def generate_prev_notional():
    """Generate a random previous notional value with ~5% chance of being zero,
    and optional rounding for variability. Returns int if whole, else float."""
    prev_notional = 0 if random.random() < 0.05 else random.randint(0, 1000)

    if random.random() < 0.5:
        divisor = random.choice([10, 100])
        decimals = random.randint(1, 2)
        prev_notional = round(prev_notional / divisor, decimals)

    # Cast to int if it's a whole number
    if isinstance(prev_notional, float) and prev_notional.is_integer():
        prev_notional = int(prev_notional)
    ...
    # Labels 0 and 1: Specific positions
        ...
        if swapType == "fx":
            ...
            if has_active_derivative:
                ...
                prev_notional = generate_prev_notional()
                prev2_notional = generate_prev_notional()
