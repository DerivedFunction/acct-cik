...
def deboost_keywords_batch(batch_id, batch_size=20, max_words_per_sentence=8, min_words_per_sentence=4):
    """
    ...
    num_label_3 = batch_size  # Adjust ratio as needed
    for _ in range(num_label_3):
        ...
        if random.random() < 0.7:
            value = round(random.uniform(1, 9999), 1)
            currency_symbol = random.choice(currency_codes)
            money_unit = random.choice(money_units)
    ...
    for _ in range(num_label_2):
        ...
        if random.random() < 0.6:
            ...
            money_unit = random.choice(money_unit_list)
...
def generate_deboost_dataset_concurrent(
    ...
    # Print statistics
    label_counts = {}
    for _, label in all_sentences:
        label_counts[label] = label_counts.get(label, 0) + 1
    
    print(f"\nTotal unique sentences: {len(all_sentences)}")
    print("Label distribution:")
    for label in sorted(label_counts.keys()):
        print(f"  Label {label}: {label_counts[label]} sentences")
