...
hedge_effectiveness_templates = [
    "{company} {verb}, both at inception and on an on-going basis, whether the derivative instruments that are used in {hedge_type} hedging transactions are highly effective in offsetting the {metric} of hedged items",
...
cp_strategy_templates = [
    "{company} entered into {swap_type} to hedge anticipated changes in {commodity} prices, resulting in {currency_code}{amount} {money_unit} of unrealized gains/losses recorded in OCI",
    "During {month} {year}, {company} executed {swap_type} to stabilize {commodity} costs for forecasted production, with {currency_code}{amount} {money_unit} in deferred fair value adjustments",
    "{company} utilized {swap_type} to manage commodity price risk on forecasted inventory, recognizing {currency_code}{amount} {money_unit} in mark-to-market gains/losses",
    ...
    "{swap_type} were executed by {company} in {month} {year} to protect against adverse price movements in {commodity}, with {currency_code}{amount} {money_unit} of unrealized gains/losses",
    "In {month} {year}, {company} entered into {swap_type} to offset commodity price volatility, recognizing {currency_code}{amount} {money_unit} in fair value adjustments",
    ...
    "During the {quarter} quarter of {year}, {swap_type} were used to manage {commodity} price risk, with {currency_code}{amount} {money_unit} of mark-to-market adjustments",
    "{company} employed {swap_type} to mitigate fluctuations in {commodity} prices, recording {currency_code}{amount} {money_unit} in OCI",
    ...
    "{company} used {swap_type} in {year} to secure {commodity} prices, recognizing {currency_code}{amount} {money_unit} in hedge-related gains/losses",
    "To manage {commodity} price exposure, {company} executed {swap_type} in {month} {year}, with {currency_code}{amount} {money_unit} in OCI adjustments",
    ...
    "{swap_type} were employed by {company} to hedge {commodity} costs, with {currency_code}{amount} {money_unit} recorded as mark-to-market adjustments in {year}",
]
cp_position_templates = [
    ...
    "During {year}, {company} executed {swap_type} with a total notional value of {currency_code}{notional} {money_unit} to hedge volatility in raw material costs",
    "As of {month} {end_day}, {year}, outstanding {swap_type} totaling {currency_code}{notional} {money_unit} are used to mitigate commodity price exposure",
...
cp_specific_templates = [
    ...
    "{swap_type} are used to hedge anticipated {commodity} purchases totaling {volume} {volume_unit} over the next {months} months",
    "In {month} {year}, {company} executed {swap_type} to lock in {commodity} prices for {volume} {volume_unit} of forecasted consumption",
...
cp_impact_templates = [
    ...
    "{company} have market sensitive instruments in the form of fixed-rate debt instruments and financial derivative instruments used to hedge our exposure to {commodity} price increases",
...
def generate_derivative_liability_paragraph(
    ...
    # ============ LABELS 6 & 7: EMBEDDED DERIVATIVES ============
        ...
        sentence = template.format(host_contract=host_contract, company=company_name, verb=random.choice(assessment_verbs))
        all_sentences.append(sentence)
    ...
    paragraph = re.sub(r"We's", "Our", paragraph, flags=re.IGNORECASE)
    paragraph = re.sub(r"\s+", " ", paragraph)  # Remove extra whitespace
    paragraph = re.sub(r"\.\.", ".", paragraph)  # Remove double periods
...
def generate_labeled_hedge_paragraph(
    ...
    if has_active_derivative is None:
        ...
        if random.random() < 0.5:
            template = random.choice(ir_fx_debt_optimization_templates)
            end_day = random.randint(28, 31)
            all_sentences.append(
                template.format(
                    company=company_name,
    ...
    # Clean up common formatting issues
    paragraph = re.sub(r"We's", "Our", paragraph)
    paragraph = re.sub(r"\s+", " ", paragraph)  # Remove extra whitespace
    paragraph = re.sub(r"\.\.", ".", paragraph)  # Remove double periods
...
def generate_sec_noise(company, currency_unit, month, year):
    ...
    phrases = [
        ...
        f"For the fiscal year ended {month} {end_day} {year}",
        f"For the transition period from {month} {end_day} {year - 1} to {month} {end_day} {year}",
        f"{company_name} (Exact Name , of Registrant as Specified in its Charter)",
    ...
    # Randomly choose 1–3 fragments
    chunks = random.sample(headers + phrases + [gibberish] + [generate_toc_line()], k=random.randint(1,3))
    return chunks
