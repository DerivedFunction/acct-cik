...
hedge_past_event_template = [
    ...
    "During the {quarter} quarter of {year}, {swap_type} were designated as {hedge_type} hedges, resulting in {currency_code}{amount} {money_unit} of unrealized losses recorded in accumulated OCI",
]
...
debt_templates = [
    ...
    "As of {month} {end_day}, {year}, {company} had total long-term debt of {currency_code}{amount} {money_unit}, consisting primarily of {debt_types}",
    "Long-term debt as of {month} {end_day}, {year} totaled {currency_code}{amount} {money_unit}, consisting of {debt_types}",
    "At year-end {year}, {company} reported total debt of {currency_code}{amount} {money_unit}, including {debt_types}",
    "{company}'s outstanding borrowings under its revolving credit facility totaled {currency_code}{outstanding} {money_unit} as of {month} {end_day}, {year}",
    # Issuances and repayments
    "During {year}, {company} issued {currency_code}{amount} {money_unit} in {debt_types} with a maturity date of {maturity_year}",
    ...
    "In {year}, {company} refinanced {currency_code}{amount} {money_unit} of existing {debt_type}, extending the maturity to {maturity_year}",
    ...
    "The weighted average interest rate on {company}'s long-term debt was approximately {pct}% as of {month} {end_day}, {year}",
    "As of {month} {end_day}, {year}, {company}'s long-term debt had a weighted average maturity of {years} years",
    ...
    "{company}'s credit agreements require maintenance of specified leverage and coverage ratios, which the company met as of {month} {end_day}, {year}",
    # Other specialized cases
    "During {year}, {company} entered into a new {currency_code}{amount} {money_unit} term loan agreement with a maturity in {maturity_year}",
    ...
    "At {month} {end_day}, {year}, unamortized debt issuance costs related to long-term debt totaled {currency_code}{amount} {money_unit}",
    "The fair value of {company}'s long-term debt was estimated at {currency_code}{amount2} {money_unit} as of {month} {end_day}, {year}",
]
...
def generate_value(haveZero=True, upperlimit=1000):
    ...
    if haveZero:
        ...
        chance = 0
    value = 0.0 if random.random() < chance else random.randint(1, upperlimit)
...
def generate_labeled_hedge_paragraph(
    ...
    def generate_hedge_policy(current=False):
        ...
        template.format(item=item, company=pick_company_name(company_name), materiality=materiality_choice)
        return sentences
    ...
    if has_active_derivative is None:
        ...
        swap_type = random.choice(swap_types) if random.random() < 0.5 else "derivatives"
        hedge_type = random.choice(hedge_types)
    ...
    # Labels 0 and 1: Specific positions
        ...
        if swapType == "fx":
            ...
            elif has_active_derivative == False:
    ...
    # Context sentences
    if include_policy:
        if has_active_derivative is not None and random.random() < 0.33:
...
def generate_accounting_noise_paragraph(
    ...
    for _ in range(2):
        ...
        for _ in range(random.randint(1, 3)):  # combine 2–4 related debt lines
    ...
    if category == "stock_price":
        ...
        ticker = string.ascii_letters[: random.randint(3, 4)].capitalize
    ...
    category = random.choice(["dividend", "repurchase"])
...
def generate_sec_noise(company, currency_unit, month, year):
    def generate_toc_line():
        ...
        return template.format(page=page_num, company=company) + " "
...
def deboost_keywords_parallel(batch_size=20, max_words_per_sentence=8, min_words_per_sentence=4):
    """
    Generate deboost sentences for derivative contexts with anti-overfitting measures.
    Sentences are longer and more realistic.
    
    Labels:
    - 2: Gen/Unknown Hedge Der. Speculative/Policy
    - 3: Irrelevant / Unrelated Context
    - 14: IR Hedge Speculative/Policy (contextless IR terms)
    - 15: FX Hedge Speculative/Policy (contextless FX terms)
    - 16: CP Hedge Speculative/Policy (contextless CP terms)
    """
    sentences = []
    ...
    accounting_buzzwords = [
        ...
        "net investment hedge", "cash flow hedge", "fair value hedge"
    ]
    ...
    for _ in range(150):
        ...
        # Build longer, more coherent sentences
        sentence_parts = []
        
        # Start with accounting terms
        if random.random() < 0.8:
        ...
        # Add more accounting terms
        sentence_parts.extend(random.sample(
            accounting_buzzwords,
            min(random.randint(1, 3), len(accounting_buzzwords))
        ))
        
        # 30% chance to add derivative terms (to prevent keyword shortcuts)
        if random.random() < 0.3:
        ...
        if random.random() < 0.7:
            ...
            currency_symbol = random.choice(currency_codes)
            money_unit = random.choice(["thousand", "million", "billion"])
            sentence_parts.append(f"{currency_symbol}{value} {money_unit}")
        
        if random.random() < 0.6:
            sentence_parts.append(f"in {random.randint(1990, 2025)}")
        
        if random.random() < 0.4:
            month = random.choice(months)
            day = random.randint(1, 28)
            year = random.randint(2015, 2025)
            sentence_parts.append(f"as of {month} {day}, {year}")
    ...
    for _ in range(120):
        ...
        # Start with generic derivative terms
        sentence_parts.extend(random.sample(
            gen_standalone,
        ...
        # 50% chance to add accounting context (makes it ambiguous with label 3)
        if random.random() < 0.5:
            sentence_parts.extend(random.sample(
                accounting_buzzwords,
                min(random.randint(1, 3), len(accounting_buzzwords))
        ...
        # 30% chance to add type-specific context (makes it ambiguous with 14/15/16)
        if random.random() < 0.3:
            type_specific_terms = random.sample([
                "interest rate", "foreign exchange", "commodity price",
                "currency", "crude oil", "natural gas", "EUR/USD", "LIBOR", "SOFR"
            ], random.randint(1, 2))
            sentence_parts.extend(type_specific_terms)
        
        # 20% chance to add specific derivative instruments
        if random.random() < 0.2:
            sentence_parts.append(random.choice(derivative_specific))
        
        # Add connectors and phrases
        if random.random() < 0.5:
            sentence_parts.append(random.choice(connectors))
            sentence_parts.append(random.choice(accounting_phrases))
        ...
        if random.random() < 0.6:
            ...
            currency_symbol = random.choice(currency_codes)
            money_unit = random.choice(["thousand", "million", "billion"])
            sentence_parts.append(f"notional {currency_symbol}{value} {money_unit}")
        
        # Add dates
        if random.random() < 0.5:
            sentence_parts.append(f"during {random.randint(1990, 2025)}")
    ...
    # --- Labels 14/15/16: Type-specific Speculative/Policy ---
    swap_types = {
        "IR": 14,
        "FX": 15,
        "CP": 16
    ...
    for prefix, label in swap_types.items():
        for _ in range(batch_size * 2):
            ...
            # Add type-specific derivative terms
            num_swaps = random.randint(2, 4)
            for _ in range(num_swaps):
                swap_type = random.choice(derivative_keywords[prefix])
                if prefix == "CP" and "commodity" in swap_type:
                    swap_type = re.sub("commodity", random.choice(commodities), swap_type)
            ...
            # 40% chance to add generic terms (prevents pure category mapping)
            if random.random() < 0.4:
                sentence_parts.extend(random.sample(
                    gen_standalone,
            ...
            # 30% chance to add accounting buzzwords
            if random.random() < 0.3:
                sentence_parts.extend(random.sample(
                    accounting_buzzwords,
                    min(random.randint(1, 3), len(accounting_buzzwords))
            ...
            # 20% chance to add terms from OTHER categories
            if random.random() < 0.2:
                other_prefix = random.choice([p for p in swap_types.keys() if p != prefix])
                cross_contamination = random.choice(derivative_keywords[other_prefix])
                if other_prefix == "CP" and "commodity" in cross_contamination:
                    cross_contamination = re.sub("commodity", random.choice(commodities), cross_contamination)
                sentence_parts.append(cross_contamination)
            
            # Add phrases and values
            if random.random() < 0.5:
                sentence_parts.append(random.choice(accounting_phrases))
            ...
            if random.random() < 0.6:
                ...
                currency_symbol = random.choice(currency_codes)
                money_unit = random.choice(["thousand", "million", "billion"])
                sentence_parts.append(f"with notional amount {currency_symbol}{value} {money_unit}")
            ...
            if random.random() < 0.3:
                ...
                sentence_parts.append(f"during the {quarter} quarter")
    ...
    # --- Longer partial phrases and realistic fragments ---
    phrase_templates = [
        "{term1} {connector} {term2} {phrase} {value}",
        "{term1} {term2} {connector} {term3} as of {date}",
        "{term1} {connector} {term2} {term3} {phrase}",
        "the Company's {term1} {connector} {term2} totaling {value}",
        "{term1} {term2} were {phrase} during {year}",
        "{term1} {connector} {term2} {phrase} {connector} {term3}"
    ]
    
    for _ in range(100):
        template = random.choice(phrase_templates)
        
        # Mix terms from different categories
        all_terms = accounting_buzzwords + gen_standalone + derivative_specific
        term1 = random.choice(all_terms)
        term2 = random.choice(all_terms)
        term3 = random.choice(all_terms)
        connector = random.choice(connectors)
        phrase = random.choice(accounting_phrases)
        
        value = generate_value()
        currency = random.choice(currency_codes)
        money_unit = random.choice(["thousand", "million", "billion"])
        value_str = f"{currency}{value} {money_unit}"
        ...
        try:
            sentence = template.format(
                ...
                value=value_str, date=date, year=year
            )
            
            # Randomly assign to label 2 or 3
            target_label = random.choice([2, 3])
            sentences.append((sentence, target_label))
        except:
            continue
...
def generate(size_per_label=100, max_workers=8):
    ...
    def submit_tasks(executor):
        ...
        for _ in range(size_per_label * 4):
    ...
    # Deboosting
    all_samples.extend(deboost_keywords_parallel(size_per_label))
