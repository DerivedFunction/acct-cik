...
def generate_labeled_hedge_paragraph(
    ...
    def generate_derivative_sentences():
        ...
        if swapType == "fx":
            if random.random() < 0.90 or not has_active_derivative:
            ...
            if random.random() < 0.90 or not has_active_derivative:
                # generic fallback
                template = random.choice(hedge_position_templates if random.random() < 0.85 else eq_position_templates)
            else:
    ...
    if include_policy or len(selected_sentences) < 2:
        if has_active_derivative is not None and random.random() < 0.33 or len(selected_sentences) < 2:
