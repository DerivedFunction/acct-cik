---
title: "Fetch URL from CIK"
format: html
editor: visual
jupyter: python3
---

# Initialization
## Installing required libraries

```{python}
# Installing required libraries
# %pip install pandas requests bs4
```

## Importing required libraries

```{python}
import pandas as pd  # Data processing
import requests      # Synchronous HTTP requests
import time          # For rate limiting
from bs4 import BeautifulSoup  # Webpage extraction
import json          # JSON parsing
import sqlite3       # To store our data
from typing import List
import random
import re # Regex expressions
```

## Initialization

```{python}
filename = "sample.csv"
DB_PATH = "webpage_results.db"
KEYWORDS = [
    # General derivative disclosures
    "derivative liability",
    "derivative liabilities",
    "embedded derivative",

    # Swaps
    "interest rate swap",
    "cross currency swap",
    "cross-currency swap",
    "total return swap",

    # Options
    "option contract",
    "options contract",

    # Forwards
    "forward contract",
    "foreign exchange contract",
    "forward foreign"

    # Other precise markers
    "notional",
    "not designated as hedging",
    "not designated as a hedging"
]
FILING_TYPES = {"10-K", "20-F"}
SEC_RATE_LIMIT = 5  # requests per second
df = pd.read_csv(filename)
df.head()
```

## Predefined Functions

## Debug printing
```{python}
debug = True # Debug printing
def debug_print(*args):
    if debug:
        print(*args)

debug_print("Debug Print status:", debug)
```

### Fetch URLs

```{python}
def rate_limit_sleep():
    """Simple sleep to respect SEC rate limits synchronously."""
    time.sleep(1 / SEC_RATE_LIMIT)


def fetch_json(url: str) -> dict | None:
    """Fetch a URL and return JSON as dict, or None if failed."""
    headers = {"User-Agent": f"testing {random.randint(1000,9999)}@example.com"}
    rate_limit_sleep()
    try:
        resp = requests.get(url, headers=headers, timeout=10)
        print("Fetching", url)
        if resp.status_code == 429:
            retry_after = resp.headers.get("Retry-After")
            if retry_after:
                print(f"Rate limited. Retrying after {retry_after} seconds.")
                time.sleep(int(retry_after))
                return fetch_json(url)  # retry
        elif resp.status_code != 200:
            print(f"Error {resp.status_code} fetching {url}")
            return None
        return resp.json()
    except Exception as e:
        print(f"Exception fetching {url}: {e}")
        return None


def extract_filings(data: dict, cik: str, name: str, ticker: str) -> List[dict]:
    """Extract filings from SEC JSON data."""
    links = []
    forms = data.get("form", [])
    accession_numbers = data.get("accessionNumber", [])
    primary_docs = data.get("primaryDocument", [])
    filing_dates = data.get("filingDate", [])
    report_dates = data.get("reportDate", [])

    for i, f_type in enumerate(forms):
        if f_type in FILING_TYPES:
            accession = accession_numbers[i].replace("-", "")
            doc = primary_docs[i]
            if not doc or doc.endswith("txt"): # For old files
                doc = f"{accession[:10]}-{accession[10:12]}-{accession[12:]}.txt"
            link = f"https://www.sec.gov/Archives/edgar/data/{cik}/{accession}/{doc}"
            links.append({
                "name": name,
                "filing_date": filing_dates[i],
                "report_date": report_dates[i],
                "link": link,
                "ticker": ticker,
                "type": f_type
            })
    return links


def get_cik_filings(cik: str) -> List[dict]:
    """Fetch all filings for a CIK (synchronously)."""
    cik = str(cik).zfill(10)
    url_main = f"https://data.sec.gov/submissions/CIK{cik}.json"

    data = fetch_json(url_main)
    if not data:
        return []

    name = data.get("name", "")
    ticker = data.get("tickers", [])[0] if data.get("tickers", []) else cik

    # Recent filings
    recent = data.get("filings", {}).get("recent", {})
    links = extract_filings(recent, cik, name, ticker)

    # Older filings
    older_files = data.get("filings", {}).get("files", [])
    for f in older_files:
        older_data = fetch_json(f"https://data.sec.gov/submissions/{f.get('name')}")
        if isinstance(older_data, dict):
            links.extend(extract_filings(older_data, cik, name, ticker))

    return links
```

### Extract Content from Webpage

```{python}
def extract_content(html: str) -> dict:
    """
    Extract paragraphs, headers, and tables from HTML.

    Returns a dict with keys:
    - 'paragraphs': list[str]
    - 'headers': list[str]
    - 'tables': list[list[list[str]]]  # tables -> rows -> cells
    """
    if not html:
        return {"paragraphs": [], "headers": [], "tables": []}
    debug_print("Parsing Output")
    soup = BeautifulSoup(html, "html.parser")

    # Paragraphs
    paragraphs = [p.get_text(strip=True) for p in soup.find_all("p") if p.get_text(strip=True)]

    # Headers
    headers = [h.get_text(strip=True) for h in soup.find_all(["h1", "h2", "h3"]) if h.get_text(strip=True)]

    # Tables
    tables = []
    for table in soup.find_all("table"):
        rows = []
        for row in table.find_all("tr"):
            cells = [cell.get_text(strip=True) for cell in row.find_all(["td", "th"])]
            if cells:
                rows.append(cells)
        if rows:
            tables.append(rows)

    return {
        "paragraphs": paragraphs,
        "headers": headers,
        "tables": tables
    }

def parse_plain_text_table(block: str):
    """
    Parse a plain-text table according to your rules:
    - Ignore <CAPTION>.
    - Columns separated by 2+ spaces.
    - First column is reserved until <S>.
    - Lines before <S> accumulate for first column.
    - Ignore separator lines (----, ====).
    """
    rows = []
    first_col_buffer = ""
    first_col_active = True  # True until we see <S>
    
    for ln in block.splitlines():
        ln = ln.rstrip()
        if not ln:
            continue
        if re.fullmatch(r"[-=\s]+", ln):
            continue  # skip separators

        if ln.strip().startswith("<CAPTION>"):
            continue  # ignore captions

        if "<S>" in ln:
            first_col_active = False
            ln = ln.replace("<S>", "").lstrip()

        if first_col_active:
            # Accumulate multi-line first-column text
            if first_col_buffer:
                first_col_buffer += " " + ln.strip()
            else:
                first_col_buffer = ln.strip()
            continue

        # Split line by 2+ spaces
        cols = [x.strip() for x in re.split(r"\s{2,}", ln)]
        if first_col_buffer:
            row = [first_col_buffer] + cols
            first_col_buffer = ""  # reset for next row
        else:
            row = cols
        rows.append(row)
    
    return rows


def extract_text(text: str):
    """
    Extract tables, headers, and paragraphs using simplified fixed rules.
    """
    tables = []
    for block_match in re.finditer(r"<TABLE>(.*?)</TABLE>", text, flags=re.DOTALL | re.I):
        block = block_match.group(1)
        rows = parse_plain_text_table(block)
        tables.append(rows)

    # Remove tables from text
    cleaned_text = re.sub(r"<TABLE>.*?</TABLE>", "", text, flags=re.DOTALL | re.I)
    
    # Split paragraphs by double newlines
    raw_paragraphs = [p.strip() for p in cleaned_text.split("\n\n") if p.strip()]
    
    # Headers = all-caps lines
    headers = [p for p in raw_paragraphs if re.search(r"[A-Z]", p) and p.upper() == p]
    paragraphs = [p for p in raw_paragraphs if p not in headers]

    return {"tables": tables, "headers": headers, "paragraphs": paragraphs}

```

### Match text

```{python}
def filter_by_keywords(content: dict) -> dict:
    """
    Filter extracted content to only include items containing keywords.
    """
    keywords_lower = [kw.lower() for kw in KEYWORDS]

    def match_text(text: str) -> list[str]:
        return [kw for kw in keywords_lower if kw in text.lower()]

    # Paragraphs
    debug_print("Matching paragraphs")
    para_matches = [(p, match_text(p)) for p in content.get("paragraphs", []) if match_text(p)]

    # Headers
    debug_print("Matching Headers")
    header_matches = [(h, match_text(h)) for h in content.get("headers", []) if match_text(h)]

    # Tables
    debug_print("Matching Tables")
    table_matches = []
    for table in content.get("tables", []):
        table_text = " ".join(cell for row in table for cell in row)
        matched = match_text(table_text)
        if matched:
            table_matches.append((table, matched))

    return {
        "paragraphs": para_matches,
        "headers": header_matches,
        "tables": table_matches
    }
```

### Process Each URL

```{python}
def fetch_url(url: str, timeout: int = 10) -> str | None:
    """
    Fetch the content of a webpage synchronously.
    """
    if not url:
        return None
    try:
        rate_limit_sleep()
        print("Fetching", url)
        resp = requests.get(url, timeout=timeout, headers={"User-Agent": "sync-fetch@example.com"})
        if resp.status_code != 200:
            print(f"Error {resp.status_code} for {url}")
            return None
        return resp.text
    except Exception as e:
        print(f"Error fetching {url}: {e}")
        return None


def process_url(url: str):
    """
    Processes a url and its data synchronously.
    """
    data = fetch_url(url)
    if not data:
        return {}
    content = None
    if url.endswith("htm"):
        debug_print("Processing as html")
        content = extract_content(data)
    else:
        debug_print("Processing as text")
        content = extract_text(data)
    filtered = filter_by_keywords(content)
    if any(filtered.values()):  # skip if nothing matches
        return filtered
    return {}
```

## Database Functions

```{python}
def create_db():
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    try:
        c.execute("""
            CREATE TABLE IF NOT EXISTS webpage_results (
                cik INTEGER,
                year INTEGER,
                url TEXT PRIMARY KEY,
                json_data TEXT
            )
        """)
        c.execute("""
            CREATE TABLE IF NOT EXISTS fail_results (
                cik INTEGER,
                year INTEGER,
                url TEXT PRIMARY KEY
            )
        """)
    except sqlite3.IntegrityError:
        debug_print("Something went wrong creating the database")
    finally:
        conn.commit()
        conn.close()


def save_result(result, df):
    """
    Save a single page result to SQLite.
    If insert fails (e.g., duplicate url), insert into fail_results.
    """
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    try:
        c.execute(
            "INSERT INTO webpage_results (url, cik, year, json_data) VALUES (?, ?, ?, ?)",
            (df.link, df.cik, df.year, json.dumps(result))
        )
    except sqlite3.IntegrityError:
        # Insert into fail_results if duplicate or other integrity error
        c.execute(
            "INSERT OR IGNORE INTO fail_results (url, cik, year) VALUES (?, ?, ?)",
            (df.link, df.cik, df.year)
        )
    conn.commit()
    conn.close()

def fetch_webpage_results():
    """
    Fetch results from webpage_results
    """
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT * FROM webpage_results")
    columns = [col[0] for col in c.description]
    rows = c.fetchall()
    pre_data = pd.DataFrame(rows, columns=columns)
    conn.close()
    return pre_data

create_db()
```

## Merge it all together

```{python}
def filter_by_fyear(filings: list[dict], fyear: int) -> list[dict]:
    """Filter filings by fiscal year."""
    return [f for f in filings if f.get("report_date") and f.get("report_date").startswith(str(fyear))]

def fetch_all_grouped(df: pd.DataFrame) -> pd.DataFrame:
    """
    Fetch filings once per CIK, filter by fiscal years, 
    and return as a new DataFrame.
    """
    records = []

    # group: {cik -> [fyears]}
    cik_groups = df.groupby("cik")["fyear"].apply(list).reset_index()

    for row in cik_groups.itertuples(index=False):
        cik = row.cik
        years = row.fyear

        filings = get_cik_filings(cik)  # fetch once

        for fyear in years:
            year_filings = filter_by_fyear(filings, fyear)
            for filing in year_filings:
                records.append({
                    "cik": cik,
                    "year": fyear,
                    **filing  # expands filing dict into columns
                })

    return pd.DataFrame(records)

def post_process(pre_data):
    # 1. Initialize lists to store the results
    all_data = []

    # 2. Loop and collect data in a simple list of dictionaries
    for row in pre_data.itertuples():
        result = json.loads(row.json_data)
        debug_print("Processing", row.cik, row.year)
        new_sentences = []
        year_string = f"The current reporting year is {row.year}. "
        for paragraph in result.get("paragraphs", []):
            # Using a simple split, better methods exist
            sentences = paragraph[0].split(".")
            for sentence in sentences:
                sentence = sentence.strip()
                # Skip empty sentences, very short ones
                if not sentence or len(sentence.split()) < 5:
                    continue
                sentence = sentence.replace("\n", " ")
                # Truncate the end if it is too long
                sentence = truncate_sentence(sentence)
                sentence = year_string + sentence
                # Check for sentence, keywords, and uniqueness
                if sentence and any(kw in sentence for kw in KEYWORDS) and sentence not in new_sentences:
                    print("Adding", sentence)
                    new_sentences.append(sentence)

        # For each row in the original dataframe, add a dictionary to our list
        # This dictionary will become a row in the new dataframe
        if new_sentences: # Only add if we found relevant sentences
            all_data.append({
                "cik": row.cik,
                "year": row.year,
                "url": row.url,
                "json_data": new_sentences # The list of sentences
            })

    # 3. Create the final DataFrame from the list of dictionaries AFTER the loop
    final_df = pd.DataFrame(all_data)
    print("Final DataFrame: ",final_df.head())
    return final_df

def truncate_sentence(sentence: str, keywords=KEYWORDS, max_clauses: int = 3, max_len: int = 250) -> str:
    if not isinstance(sentence, str):
        return sentence

    # Normalize for matching
    lower_sentence = sentence.lower()

    # Split by comma, semicolon, or period
    parts = re.split(r'[;,\.]', sentence)

    # Start with first few chunks
    truncated = " ".join(parts[:max_clauses]).strip()

    # Ensure keywords are preserved
    found_kw = None
    for kw in keywords:
        if kw.lower() in lower_sentence:
            found_kw = kw
            break

    if found_kw and found_kw.lower() not in truncated.lower():
        # Find clause that contains the keyword
        for part in parts:
            if found_kw.lower() in part.lower():
                truncated += " " + part.strip()
                break

    # If still too long, cut at max_len safely
    if len(truncated) > max_len:
        truncated = truncated[:max_len].rsplit(" ", 1)[0] + "..."

    return truncated
```

# Execute Main Loop
```{python}
report_results = fetch_all_grouped(df)
```

## Pre-Process each link and store in a database
```{python}
for report in report_results.itertuples(index=False):
    debug_print("Processing", report.cik, report.year)
    result = process_url(report.link)
    save_result(result, report)
```

## Post-Process
### Prefetch from database
```{python}
pre_data = fetch_webpage_results()
pre_data.head()
```

### Post-Process and store in database
```{python}
post_data = post_process(pre_data)
post_data.to_excel("post_results.xlsx", index=False)
```