...
def save_post_result(df: pd.DataFrame):
    """
    Saves the entire dataframe to SQLite using the efficient df.to_sql() method.
    """
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    for row in df.itertuples(index=False):
        try:
            c.execute("""
                INSERT INTO post_results (cik, year, url, json_data)
                VALUES (?, ?, ?, ?)
            """, (row.cik, row.year, row.url, json.dumps(row.json_data)))
            conn.commit()
        except sqlite3.IntegrityError:
           print("Something went wrong. Is it a duplicate?")
    conn.close()
