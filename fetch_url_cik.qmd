...
def filter_by_keywords(content: dict, year: int) -> dict:
    ...
    para_matches = [p for p in content.get("paragraphs", []) if match_text(p)]

    match_sentences = []
    year_string = f"The current reporting year is {year}. "
    ...
    for paragraph in para_matches:
        ...
        if not sentences or len(sentences.split()) < 6:
            continue
        print(sentences)
        for sentence in sentences:
            final_sentence = year_string + sentence
            if match_text(final_sentence) and final_sentence not in match_sentences:
...
```{python}
pre_data.to_excel("post_results.xlsx", index=False)
```