...
def save_post_result(result, df):
    """
    ...
    try:
        c.execute(
            "INSERT INTO post_results (url, cik, year, json_data) VALUES (?, ?, ?, ?)",
            (df.link, df.cik, df.year, json.dumps(result))
        )
    except sqlite3.IntegrityError:
        debug_print("Something went wrong saving the database", json.dumps(result), df)
...
def post_process():
    ...
    conn.close()
    return pre_data
pre_data = post_process()
pre_data.head()
```
```{python}
for row in pre_data.itertuples(index=False):
    result = json.loads(row.json_data)
    # For each paragraph, extract only the sentences with the keyword, and discard the rest
    new_sentences = []
    for paragraph in result.get("paragraphs", []):
        sentences = paragraph.split(".").strip()
        for sentence in sentences:
            if any(kw in sentence for kw in KEYWORDS):
                new_sentences.append(sentence + "\n. The current reporting year is " + str(row.year))
    # Create a new dataframe with cik, year, and new_sentences
    new_df = pd.DataFrame({"cik": row.cik, "year": row.year, "sentences": new_sentences})
    save_post_result(new_df, row)
```